...
class SearchMultiProcessEnv:
    """Vectorized wrapper over N SearchEnv sub-environments.

    NOTE: the original (verl-agent) version inherited ``gym.Env``, but it never
    used any gym API (no action/observation spaces, no registration). We drop
    the gym dependency here so the example is self-contained — skyrl_gym's own
    ``Env`` base class is used by the underlying SearchEnv and needs no gym.
    ...
    def __init__(
        ...
        from examples.skillrl.env_package.search.third_party.skyrl_gym.envs.search.env import SearchEnv
